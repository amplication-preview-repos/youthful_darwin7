/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { SavingsAccountWhereInput } from "./SavingsAccountWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { SavingsAccountOrderByInput } from "./SavingsAccountOrderByInput";

@ArgsType()
class SavingsAccountFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => SavingsAccountWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => SavingsAccountWhereInput, { nullable: true })
  @Type(() => SavingsAccountWhereInput)
  where?: SavingsAccountWhereInput;

  @ApiProperty({
    required: false,
    type: [SavingsAccountOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [SavingsAccountOrderByInput], { nullable: true })
  @Type(() => SavingsAccountOrderByInput)
  orderBy?: Array<SavingsAccountOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { SavingsAccountFindManyArgs as SavingsAccountFindManyArgs };
